/**
 * Spot type information and utilities.
 */

export const spotInfo: Record<number, string> = {
  0: '',
  1: 'Start',
  2: 'Battle Avoid',
  3: 'Obtain Resources',
  ...
  11: 'Long Distance Aerial Battle',
  12: 'Manual Selection',
  13: 'Aerial Recon',
  14: 'Night Battle',
...
export const spotIcon: Record<string, string | number> = {
  Battle: '4-1',
  'Night Battle': '4-1',
  'Boss Battle': 5,
  'Obtain Resources': 2,
...
/**
 * Determine spot kind from api_event_id and api_event_kind.
 */
export function getSpotKind(api_event_id: number, api_event_kind: number): number {
  if (api_event_id === 4) {
    if (api_event_kind === 2) return 14
    if (api_event_kind === 4) return 8
    if (api_event_kind === 5) return 15
    if (api_event_kind === 6) return 11
  }
  if (api_event_id === 6) {
    if (api_event_kind === 1) return 7
    if (api_event_kind === 2) return 12
  } else if (api_event_id === 7) {
    if (api_event_kind === 0) return 13
...
const spotMessages: Record<number, string> = {
  0: '気のせいだった。',
  1: '敵影を見ず。',
  3: '穏やかな海です。',
  4: '穏やかな海峡です。',
  ...
  12: '艦隊、増速！これより連合艦隊は艦隊決戦を行う！　我に続け！',
  ...
  17: 'KdMB機動部隊本隊小沢艦隊は敵機動部隊主力を北方に誘引、好機を捉えこれを捕捉撃破す！',
...
export function getSpotMessage(api_event_id: number, api_event_kind: number): string {
  if ([1, 6].includes(api_event_id)) {
    return spotMessages[api_event_kind] ?? ''
  }
  return ''
}
